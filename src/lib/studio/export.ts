...
import type { ExportSettings, CardSettings } from "./types";
...
function getExportDimensions(card: CardSettings, resolution: number, even = false) {
  const widthMm = Number(card.widthMm);
  const heightMm = Number(card.heightMm);
  ...
  if (
    ...
    // Fallback to the original standard card ratio.
    const height = resolution;
    let width = Math.round(resolution * (63 / 88));
...
export async function exportStill(
  ...
  const { width: w, height: h } = getExportDimensions(card, res);

  const transparent =
    settings.transparent || settings.background === "transparent";
...
export async function exportVideo(
  ...
  const { width: w, height: h } = getExportDimensions(card, res, true);
  ...
  if (!mime || typeof MediaRecorder === "undefined") {
    throw new Error(
      "Video recording is not supported in this browser. Export a GIF instead.",
    );
  }
